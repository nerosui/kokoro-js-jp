...
辞書やボイスを差し替えたい場合は`japanese: { dicUrl, voiceUrl }`で上書きできる。`dicUrl`は
(アーカイブではなく)`sys.dic`等8ファイルを個別に配信する**ディレクトリ**URL、`voiceUrl`は単一の
`.htsvoice`ファイルURLを指定する(詳細は`src/g2p/japanese.ts`の`JapaneseG2PConfig`を参照)。
